...
- **Cost, measured:** 10 authenticated requests (5 searches, the sentinel, contractor metrics twice, coverage, version) and **5 billable credits** per binary; a binary predating an assertion's version skips it and makes one fewer. Resolving a supplied binary's version reaches no API — with no key in its scratch HOME, `version` reports the build stamp alone. A search costs 1 credit, a 422 costs 0, and contractor metrics, coverage, and `/meta/release` are credit-exempt. The OpenAPI canary is unauthenticated, free, and runs once in its own job rather than per binary.
- `SHOVELS_TEST_BINARY=/path/to/shovels` runs the suite against a downloaded release instead of a build of HEAD. That is the point: a suite that only tests HEAD stays green while the binary users are running is broken, which is exactly what happened with v0.8.0.
- **An assertion about output the CLI began producing in a known version names that version.** It is false for every release published earlier and true from that version on — which is how a healthy v0.8.4 spent three nights filing `contract-drift` issues against itself. Those cases call `requireVersionAtLeast` and skip with a reason, so the lane states what it did not check. The gate is the version, never whether the binary was downloaded: skipping every download would retire the assertion from the released lane permanently, including for the releases that satisfy it. Output shape older than the oldest release the lane runs, and everything about what the API accepts and returns, is asserted everywhere.
